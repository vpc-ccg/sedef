	...
	const Hit &orig)
{
	...
	for (auto &a: anchors) {
		score.push_back(
			+ Globals::Chain::Refine::MATCH * a.aln.matches()
	...
	for (int ai = 0; ai < anchors.size(); ai++) {
		if (same_chr) {
			...
			if ((rhi - rlo) - qo < Globals::Chain::Refine::SIDE_ALIGN && (qhi - qlo) - qo < Globals::Chain::Refine::SIDE_ALIGN) { // no gap between 
				continue;
			}
		}
		...
		for (int aj = ai - 1; aj >= 0; aj--) {
			...
			int mis = Globals::Chain::Refine::MISMATCH * mi, 
				gap = Globals::Chain::Refine::GAPOPEN + Globals::Chain::Refine::GAP * (ma - mi);
	...
	for (auto &m: maxes) {
		...
		if (est_size < Globals::Chain::Refine::MIN_READ - Globals::Chain::Refine::SIDE_ALIGN) {
		...
		for (auto &h: hits) {
			...
			if (qhi-qlo-qo < Globals::Chain::Refine::SIDE_ALIGN && rhi-rlo-ro < Globals::Chain::Refine::SIDE_ALIGN) {
		...
		hit.aln = Alignment(hit.query->seq, hit.ref->seq, guide, Globals::Chain::Refine::SIDE_ALIGN);
