...
void print_help() {
  eprn("Basic usage: sedef [command] [args...]\n"
       "All meaningful output is redirected to stdout. \n \n"
       "It's recommended to use sedef.sh unless you really know what you are "
       "doing. "
       "List of commands: \n"
       "- search [reference.fa] [chrQuery] [chrRef] \n"
       "  finds all initial SDs by aligning chrQuery onto the chrRef. \n"
       "  reference.fa should be soft-masked (i.e. all common repeats are in "
       ...
       "    -u, --uppercase \n"
       "      set minimum number of uppercase (non-masked) letters in the seed "
       "hit (default: 12) \n"
       "    -e, --error \n"
       ...
       "    -t, --threads \n"
       "      number of threads (providing this will activate parallel mode--- "
       "use sedef.sh unless really sure) \n"
       "    -m, --max-parallel-size (default: 0)\n"
       "      maximum size of chromosome in MB to be processed by parallel "
       "mode (default: no limit)\n"
       "- align bucket -n [count] [bed_directory(/)] [buckets/] \n"
       "  bucket BEDs from [bed_directory] directory (or file) in the [count] "
       "bins (files) \n"
       ...
       "  WARNING: takes a lot of time--- run in parallel via sedef.sh if "
       "possible. \n"
       "  params: \n"
       "    -k, --kmer \n"
       "      set seeding k-mer size (not necessarily equal to search k-mer "
       "size) \n"
       "    --match, --mismatch, --gap-open, --gap-extend \n"
       "      set alignment parameters for initial seed alignment (default: 5, "
       "-4, -40, -1) \n"
       "    --extend-ratio, --max-extend \n"
       "      set parameters for seed SD heuristic extension (default: 5, "
       "15000) \n"
       ...
       "  params: \n"
       "    --max-ok-gap \n"
       "      maximal allowed gap size expressed as percentage of SD length "
       "(default: 50) \n"
       "    --min-split \n"
       "      minimal allowed SD length after splitting large gaps (default: "
       "1000) \n"
       "    --uppercase \n"
       "      minimum amount of uppercase (non-masked) characters in SD "
       "(default: 100) \n"
       "    --max-error \n"
