...
const int MIN_READ_SIZE = 10000;
// const int SKETCH_SIZE = 2 * MIN_READ_SIZE / WINDOW_SIZE;
...
inline double tau(double error = .15)
{
...
// 12	chr1	92387	136258	chr1:243174377	0	+	chr1	243174377	243218157	43780	...	43446	42679	767	507	260	0.982346	0.980338	0.017865	0.017891	43871
	...
	auto mapping = search(91256, hash.second, hash.first);
		t_end = chrono::high_resolution_clock::now();
		print("time: {:.2f}s\n", chrono::duration_cast<chrono::milliseconds>(t_end - t_start).count() / 1000.00), t_start = t_end;

	print("{} (tau={})\n", mapping.first.size(), ::tau()); //3654?
	for (auto &p: mapping.first) if (p.first != -1) {
		print("mapping {}: {} {} {:.2f}\n", p.first, p.second, p.second/mapping.second, j2md(p.second/mapping.second));
	}
